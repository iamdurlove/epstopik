...
$(document).ready(function () {
	...
	$("input[type='radio']").click(function () {
		...
		$("input[name='sample-radio'][value=" + radioValue + "]")
			.parent()
			.addClass("bg-success-green");
		...
		if (
			...
			if (window.localStorageData[window.count] != null) {
				$.each(window.localStorageData, function (key, val) {
					if (key == window.count) {
						window.localStorageData[key] = Object.assign(
							res[window.count],
							obj
						);
						localStorage.setItem(
							"review",
							JSON.stringify(window.localStorageData)
						);
					}
				});
			} else {
...
$(document).on("click", "#ar-right", function () {
	if (QUE < window.count) {
		...
		if (
			$("input[name='sample-radio']:checked").val() ==
			res[window.count]["answer"].toLowerCase()
		) {
			window.correct = window.correct + 1;
			var correct_percentage =
				(window.correct * 100) / window.total_question_percentage;
		} else {
			window.incorrect = window.incorrect + 1;
			var incorrect_percentage =
				(window.incorrect * 100) / window.total_question_percentage;
		}
	}

	if (window.current_question >= window.limit) {
		$("#ar-right").html(
			'<div class="text-center mt-1" id="resultData">Submit</div>'
		);
	} else {
		...
		$.each(window.bookmarked_list, function (key, val) {
			if (key == window.count) {
				if (val["userans"] != "") {
					$("input[name='sample-radio']")
						.parent()
						.removeClass("bg-success-green");
					$("input[name='sample-radio'][value=" + val["userans"] + "]")
						.parent()
						.addClass("bg-success-green");
				}
			}
...
$(document).on("click", "#resultData", function () {
	...
	swal({
		...
		if (value) {
			...
			window.bookmarked_list = JSON.parse(localStorage.getItem("review"));
			window.exit = true;
			$.redirect("self-challenge-result.php", {
				bookmarked_list: window.bookmarked_list,
				currect_ans: window.correct,
				wrong_ans: window.incorrect,
				play_time: TIME_TAKE,
				challage_time: window.challage_time,
			});
		}
	});
});
...
function loadQuestions(res) {
	if (window.attempt >= window.limit) {
		...
		if (window.count == 0) {
			...
			$("#ar-left").html("<button>Previous Question</button>");
		}
		$("input[name='sample-radio']").parent().removeClass("bg-success-green");
		$(".progress").empty();
		...
		if (window.current_question >= window.limit) {
			...
			$("#ar-right").html(
				'<div class="text-center mt-1" id="resultData"> <button>Submit</button> </div>'
			);
		} else {
			$("#ar-right").children().css("display", "block");
			$("#ar-right").html("<button>Next Question</button>");
		}
		...
		if (res[window.count]["question_type"] == "1") {
			...
			if (is_option_e_mode_enabled) {
				if (
					res[window.count]["optione"] != "" &&
					res[window.count]["optione"] != null
				) {
		...
		if (res[window.count]["image"] != "") {
			var html =
				"<img class='my-questions-img' src='" +
				res[window.count]["image"] +
				"' width='" +
				QUIZ_IMG_WIDTH +
				"'>";
